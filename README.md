...
- **实时切换**: 在页面右上角点击语言切换按钮，无需刷新页面
- **持久化存储**: 语言偏好自动保存到本地存储，下次访问时自动应用
- **全面翻译**: 包括所有界面文本、提示信息、表格标题等
- **动态内容**: 动态生成的内容（如统计数据、API 密钥列表等）也会根据语言设置显示
