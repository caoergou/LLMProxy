...
class ApiCallModel {
    static create(callData) {
        return new Promise((resolve, reject) => {
            ...
            db.run(`INSERT INTO api_calls 
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [callData.api_key_id, callData.endpoint, callData.method,
                ...
                callData.tokens_per_second || 0, callData.model_name || null, callData.provider_name || null], function (err) {
                if (err) {
                    reject(err);
                }
                else {
    ...
    static getPerformanceMetrics(timeRange = '24h') {
        return new Promise((resolve, reject) => {
            ...
            db.all(`SELECT 
                    ...
                    ORDER BY total_tokens_consumed DESC`, [timeRange], (err, rows) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    static getTokenUsageStats(timeRange = '24h') {
        return new Promise((resolve, reject) => {
            const db = Database_1.default.getDb();
            if (!db) {
                reject(new Error('Database not initialized'));
    ...
    static getHourlyMetrics(hours = 24) {
        return new Promise((resolve, reject) => {
            const db = Database_1.default.getDb();
            if (!db) {
                reject(new Error('Database not initialized'));
            ...
            db.all(`SELECT 
                    ...
                    ORDER BY hour`, [`-${hours} hours`], (err, rows) => {
                if (err) {
                    reject(err);
                }
                else {
