...
class ApiKeyModel {
        ...
        return new Promise((resolve, reject) => {
            ...
            db.get(`SELECT * FROM api_keys WHERE id = ?`, [id], (err, row) => {
                if (err) {
                    ...
                    row.api_key = Encryption_1.default.decrypt(row.api_key);
                    if (row.api_secret) {
                        row.api_secret = Encryption_1.default.decrypt(row.api_secret);
                    }
                    resolve(row);
                }
                else {
        ...
        return new Promise((resolve, reject) => {
            ...
            db.all(`SELECT id, provider, name, cost_per_request, remaining_quota, 
                    ...
                    ORDER BY 
                        ...
                        cost_per_request ASC`, [provider], (err, rows) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    static create(keyData) {
        return new Promise((resolve, reject) => {
            ...
            const encryptedApiKey = Encryption_1.default.encrypt(keyData.api_key);
            const encryptedApiSecret = keyData.api_secret ? Encryption_1.default.encrypt(keyData.api_secret) : null;
            db.run(`INSERT INTO api_keys 
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [keyData.provider, keyData.name, encryptedApiKey, encryptedApiSecret,
                keyData.base_url, keyData.cost_per_request || 0,
                keyData.remaining_quota || -1, keyData.total_quota || -1,
                keyData.is_active !== false ? 1 : 0], function (err) {
                if (err) {
                    reject(err);
                }
                else {
    ...
    static update(id, keyData) {
        return new Promise((resolve, reject) => {
            ...
            if (keyData.api_key !== undefined) {
                ...
                updateValues.push(Encryption_1.default.encrypt(keyData.api_key));
            }
            if (keyData.api_secret !== undefined) {
                updateFields.push('api_secret = ?');
                updateValues.push(keyData.api_secret ? Encryption_1.default.encrypt(keyData.api_secret) : null);
            }
            ...
            db.run(`UPDATE api_keys SET ${updateFields.join(', ')} WHERE id = ?`, updateValues, function (err) {
                if (err) {
                    reject(err);
                }
                else {
    ...
    static updateQuota(id, remainingQuota) {
        return new Promise((resolve, reject) => {
            ...
            db.run(`UPDATE api_keys SET remaining_quota = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [remainingQuota, id], function (err) {
                if (err) {
                    reject(err);
                }
                else {
