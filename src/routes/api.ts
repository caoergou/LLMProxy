    ...
    const capabilities = providers.map((provider) => {
      ...
      return {
        ...
        supported_features: {
          ...
          function_calling: models.some(
            (m) => m.capabilities.supports_function_calling
          ),
...
router.post("/v1/chat/completions", async (req: Request, res: Response) => {
  try {
    ...
    if (openaiRequest.stream) {
      const result = await OpenAIUnifiedService.chatCompletionsStream(
        openaiRequest,
        res,
        {
          preferredProvider,
      ...
      if (!result.success && result.error) {
        if (!res.headersSent) {
          const status =
            result.error?.error?.type === "invalid_request_error" ? 400 : 500;
...
router.get("/v1/models/:model", async (req: Request, res: Response) => {
  try {
    ...
    if (result.success) {
      ...
      const status =
        result.error?.error?.type === "invalid_request_error" ? 404 : 500;
      res.status(status).json(result.error);
    }
  } catch (error) {
    res.status(500).json({
...
router.get("/models-view", async (req: Request, res: Response) => {
  try {
    ...
    if (min_context_length)
      capabilities.context_length = parseInt(min_context_length as string);
    ...
    try {
      ...
      switch (sort_by) {
        ...
        case "availability":
          sortedProviders.sort((a, b) => {
            const compare =
              a.performance.availability - b.performance.availability;
            return order === "desc" ? -compare : compare;
          });
          break;
      }
...
router.get(
  "/models-view/:family/best-provider",
  async (req: Request, res: Response) => {
    try {
      const family = req.params.family;
      const {
      ...
      const selectedCriteria =
        criteriaMap[criteria as keyof typeof criteriaMap] || criteriaMap.price;
      const bestProvider = ModelAggregationService.getBestProvider(
        family,
        selectedCriteria
      );
